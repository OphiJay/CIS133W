...
function MultipleChoiceQuestion(title, text, options) {
    ...
    this.toHTML = function() {
        ...
        for (var i = 0; i < options.length; i++) {
            result += '<li><input type="radio" name="' + 
                        title + '" value="' + 
                        options[i] + '">' +
                        options[i] + '</li>\n';
        }   
...
function MultipleChoiceOtherQuestion(title, text, options) {
    ...
    this.toHTML = function() {
        ...
        result += '<p>Other: <input id="' + title + '_other" ';
        result += 'type="text" name"Other"></p>\n';
        return result;
    };
    this.getAnswers = function () {
...
function ShortAnswerQuestion(title, text) {
    ...
    this.toHTML = function() {
        ...
        result += '<p>Answer: <input id="' + title + '" type="text" name"' +
                     title + '"></p>\n';
        return result;
    };
    this.getAnswers = function () {
...
function QuestionSection(title, questions) {
    this.title = title;
    this.toHTML = function () {
        var result = '<hr><h2>Section ' + this.number + ': ' + title + '</h2>\n';
...
function Survey(title, sections) {
    this.toHTML = function () {
        ...
        result += '<hr><input id="submit" type="button" value="SUBMIT">\n';
        return result;
    };
    this.getAnswers = function () {
        var answers = {};
...
window.addEventListener("load", function() {
    ...
    function submit(){
        ...
        form.method = 'POST';
        form.action = 'submit_survey_lab05.php';
        form.innerHTML = '<input type="hidden" name="answers" value="' + 
